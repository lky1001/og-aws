...
-	🔹**EC2 vs. 다른 서비스:** AWS 의 대부분의 유저는 [EC2](#ec2)(AWS의 주력 가상 서버 제품)과 친숙 하다. 그리고 그 외에 S3와 CLB 등이 있다. 그러나 AWS 의 제품들은 이제 기본적인 Iaas 를 넘어서 확장 되었다. 그리고 회사들은 종종 많은 AWS 서비스들에 대해서 잘 이해하지 못하거나, 그 가치를 알지 못하고 어떻게 적용할 수 있는지 알지 못한다. 그 이유로는 서비스의 종류, 참신함, 복잡함, 브랜드 혼동, AWS 독점기술에 대해 ⛓ 묶이는 것에 대한 공포심이  [급격히 증가중](#which-services-to-use) 이기 때문이다. 비록 좀 어려울 수 있지만, 회사 내 기술 의사결정권자들은 AWS 서비스를 넓게 이해하고 자세한 정보에 근거한 결정을 내리는 것이 중요하다. (우리는 이 가이드가 도움이 되길 바랍니다)
-	🚪**AWS vs. 다른 클라우드 제공자:** AWS 가 가장 점유율이 높은 IaaS 제공자 이지만, (31% 점유율 [2016 추정](https://www.srgresearch.com/articles/aws-remains-dominant-despite-microsoft-and-google-growth-surges)), 몇몇 회사에는 더 상황에 잘 맞는 경쟁자 / 대체자가 있다. [이 가트너 보고서](https://www.gartner.com/doc/reprints?id=1-2G2O5FC&ct=150519&st=sb) 에 다른 메이저 클라우드 제공자에 대한 좋은 오버뷰가 있다:
	-	[**Google Cloud**](https://cloud.google.com/)는 AWS 보다 후발 주자이지만, 많은 리소스를 갖고있으며 (큰 회사들을 포함)  많은 회사들에 의해 사용되고 있으며 점유율을 높여가고 있다. 모든 AWS 서비스들이 Google Cloud 와 비슷한 서비스를 갖고 있지는 않다. 반대로도 역시 Google 은 특히 다음과 같은 머신러닝 기반의 진보된 서비스를 제공한다. [비전(Vision)](https://cloud.google.com/vision/), [대화(Speech)](https://cloud.google.com/speech/), [자연어(Natural Language)](https://cloud.google.com/natural-language/) 만약 이미 AWS 를 사용하고 있다면, Google Cloud 로 전환하기는 어렵지만, 실제 이런일이 벌어지고 있다 : [Spotify 이전 사례](http://www.wsj.com/articles/google-cloud-lures-amazon-web-services-customer-spotify-1456270951) 여기에 상대적 이점에 대해 더 많은 논의가 진행 되고 있다: [Quora](https://www.quora.com/What-are-the-reasons-to-choose-AWS-over-Google-Cloud-or-vice-versa-for-a-high-traffic-web-application)
	-	[**Microsoft Azure**](https://azure.microsoft.com/en)는 사실 Microsoft 스택을 주로 사용하는 회사들을 위한 선택이지만, Linux 에 대한 지원도 강조되고 있다.
	-	**중국**의 경우, AWS의 영역은 비교적 작다. 시작은 알리바바의 [Aliyun](https://intl.aliyun.com/)이 점유 하고 있다.
	-	매우 큰 회사들의 경우 인프라를 관리하는 비용을 줄이고 싶을 수 있다. 예를 들어, [Dropbox](https://news.ycombinator.com/item?id=11282948)가 그들의 인프라를 구축해서 이전 했다.
	-	다른 클라우드 제공자 (예를 들어 [Digital Ocean](https://www.digitalocean.com/))도 때로는 매우 편리한 사용성과 개인화된 지원, 낮은 가격으로 비슷한 서비스를 제공한다.  그러나 어떤 업체도 AWS 가 제공하는 넓이의 제품이나 mind-share, 시장 점유율을 갖지는 못했다.
	-	전통적인 호스팅 업체 (예를 들어 [Rackspace](https://www.rackspace.com/)) 들도 클라우드 솔루션을 제공한다.
-	🚪**AWS vs. PaaS:** 만약 당신이 원하는것이 단지 비교적 간단한 하나의 서비스를 운영하는 것이라면, 그리고 운영에 최소한의 시간을 사용하기 윈한다면, [platform-as-a-service](https://en.wikipedia.org/wiki/Platform_as_a_service) 를 고려하라. (예를 들어 [Heroku](https://www.heroku.com/)) AWS를 PaaS 처럼 사용하기 위한 방법인 Elastic Beanstalk는 확실히 간단한 유즈케이스에 사용하기엔 복잡하다.
-	🚪**AWS vs. 웹 호스팅:** 만약 당신의 목표가 웹사이트나 블로그를 호스팅 하는것이라면, 그리고 당신이 앱을 만들거나 복잡한 서비스를 기대하지 않는다면, 많은 [web hosting services](https://www.google.com/search?q=web+hosting) 중 하나를 고려해 보자.
-	🚪**AWS vs. 매니지드 호스팅(managed hosting):** 전통적으로, 많은 회사들이 [managed hosting](https://en.wikipedia.org/wiki/Dedicated_hosting_service) 제공자에 물리적 서버를 유지보수 하기 위해 지불 한다. 그리고 그들의 소프트웨어는 임대된 하드웨어에 빌드되고 배포되어 진다. 이것은 비지니스 관점에서 이해가 되는일인데, 법적, 성능, 컴플라이언스 제한 때문에 직접 하드웨어를 제어해야 될 필요가 있을 수 있기 때문이다. 그러나 이것은 개발자 중심의 스타트업이나 젊은 기술 기업들은 구식 방법 또는 불필요한 일로 생각한다.
-	**복잡도:** AWS는 당신이 시스템을 가장 큰 규모까지 빌드하고 확장할 수 있게 한다. 그러나 확장된 서비스의 복잡도는 상당한 깊이의 지식과 경험을 요구 한다. 비록 매우 간단한 유즈케이스를 AWS에서 "정확히" 사용하기 위해서는 더 간단한 환경 (Heroku 나 Digital Ocean같은) 을 사용하는 것보다 더 많은 지식이 필요할 수 있다.
-	**지리적 위치:** AWS의 데이터 센터는 [12개도 넘는 위치](http://docs.aws.amazon.com/AWSEC2/latest/UserGuide/using-regions-availability-zones.html#concepts-available-regions)에 있다. 이는 **리전(regions)**이라고 불리며, 유럽, 동아시아, 북아메리카, 남아메리카, 호주와 인도에 있다. 또한 더 많은 **엣지 로케이션(edge locations)** 들이 전세계에 있는데, 이것은 CloudFront 같은 서비스들의 지연을 감소시키기 위해 있다.
	-	[현재 목록](https://aws.amazon.com/about-aws/global-infrastructure/) 에 신규 지역을 포함한 regions 과 edge locations 들이 나와있다.
	-	만약 당신의 인프라가 지연이나 처리량의 이유로 물리적으로 다른 서비스와 근접해야될 필요가 있다면(예를 들어 광고 교환 지연시간), 지역에 따라 AWS 사용 가능성이 달라질 것이다.
-	⛓**락인(Lock-in):** AWS를 사용한다면, 당신이 의존하는 AWS 서비스들에 대해 다른 대안이 없다는것을 알고있는것이 중요하다.
	-	락인(Lock-in) 은 당신의 회사에 완전히 문제가 없을 수도 있고, 큰 리스크가 될 수도 있다. 비지니스 관점에서 이 선택을 명시적으로 하는것이 중요하다. 그리고 AWS 에 묶이는 비용, 운영, 사업 지속성, 경쟁 리스크를 고려하라. AWS 는 가장 유력하고 믿을만한 업체이다. 많은 회사들이 AWS 를 최대한 사용하는것에 만족하고 있다. 다른 쪽으로는 다음 이야기를 참고하라 : [비용이 증가할때 "클라우드 감옥"의 위험](http://firstround.com/review/the-three-infrastructure-mistakes-your-company-must-not-make/).
	-	일반적으로, AWS 서비슬ㄹ 많이 이용할수록 AWS 에 많이 묶인다. 이것은 미래에 다른 업체로 이전할때 더 많은 엔지니어링 리소스 (시간과 비용)이 든다는 것이다.
	- 가상 서버나 표준 데이터베이스와 같은 기본 서비스들은 다른 클라우드 업체나 on-premise 로 옮기기 쉽다. 로드 밸런서나 IAM 같은 서비스들은 AWS 고유 서비스 이지만, 다른 업체들에도 비슷한 서비스가 있다. 주요하게 고려해야될 것은, 엔지니어들이 시스템을 오픈소스가 아니거나 비교적 교체하기 어려운 AWS 서비스들로 설계하는지 이다. 예를들어 Lambda, API Gateway, Kinesis, Redshift, 그리고 DynamoDB 는 오픈소스나 상업 서비스에서 실질적인 대체 가능한 서비스가 없다. 그에 비해 EC2, RDS (MySQL 또는 Postgres), EMR, 그리고 ElastiCache 등은 가능하다. ([아래 내용](#which-services-to-use) 참조, 이 서비스들은 ⛓. 표기가 되어있다)
...
-	대부분의 고객들이 몇 가지의 서비스들을 심도 있게 또는 가볍게 사용하고 있으나, 대부분의 서비스들은 아예 사용하지 않고 있습니다. 어떠한 서비스를 사용할지는 케이스에 따라 다르며, 서비스 선택은 회사에 따라 상당한 차이를 보입니다.
-	**미숙하고 인기 없는 서비스:** AWS에서 유망해 보이는 서비스라고 해서 독자가 무조건 해당 서비스를 사용할 필요는 없습니다. 어떤 서비스들은 적용 범위가 제한적이고, 기술적으로 성숙하지 않을 수 있고, 지나치게 편향되거나 제한적일 수 있습니다. 따라서 독자 자신만의 솔루션을 쌓아 가는 것이 더 나을 수 있습니다. 우리는 제품군을 카테고리화하여 독자들이 자신만의 솔루션을 구축해 나가는 것을 돕고자 합니다.
-	**필수로 알아야 할 인프라:** 전형적인 중소 규모의 사용자들은 가장 먼저 다음과 같은 서비스들에 집중할 것입니다. 당신이 AWS 시스템을 관리하는 경우, 아래 모든 서비스들에 대하여 조금이라도 알 필요가 있습니다. (당신이 이 서비스들을 직접 사용하지 않더라도, 어떤 서비스를 사용할 지 현명하게 선택할 수 있을 정도는 배워야 합니다.)
...
-	**선택사항이지만 중요한 인프라:** 아래는 덜 알려져 있고 덜 사용되고 있으나 중요하고 유용한 인프라 요소들입니다. 물론 다른 대안 요소들을 사용하는 독자 나름의 타당한 이유가 있을 수 있으므로, 아래 요소들이 당신의 요구 사항을 충족시킬 수 있는 지 면밀히 검증하기 바랍니다:
	...
	-	⛓🕍[ECS](#ecs): 도커 컨테이너/클러스터 관리 (도커는 ECS 없이 직접 사용 가능합니다.)
	-	⛓[ECR](https://aws.amazon.com/ecr/): 사설 도커 레지스트리 서비스
	-	🐥[Config](https://aws.amazon.com/config/): AWS 리소스 인벤토리, 구성 기록, 구성 변경 알림 서비스
-	**특수 인프라:** 이 서비스들은 특정 유즈 케이스에 맞춰져 있으므로 당신의 상황에 잘 적용시킬 수 있는 지 확인이 필요합니다. 또한 대다수가 등록 아키텍처(proprietary architectures)여서 당신의 제품과 AWS의 연관성을 높이는 경향이 있습니다.
	-	⛓[DynamoDB](#dynamodb): 저 지연(Low-latency) NoSQL 키-밸류 저장소
	-	⛓[Glacier](#glacier): S3보다 느리지만 저렴한 대체 요소
	...
	-	⛓[IoT](https://aws.amazon.com/iot/): AWS와 클라이언트 사이에서 HTTP와 WebSockets, MQTT을 기반으로 양방향 통신을 해주는 서비스 (항상 필요하지는 않으나 주로 기기나 센서 같은 "사물 (thing)")
	...
	-	[Trusted Advisor](https://aws.amazon.com/premiumsupport/trustedadvisor/): 비용 절약 및 개선을 위한 자동화된 팁 서비스
-	**복합(Compound) 서비스:** 복합 서비스들은 대부분 특정 목적에 따라 이용되지만, 복잡한 문제의 해결이 가능하여 사용자를 서비스 생태계에 유인할 수 있는 완전히 발달된 서비스입니다. 서비스의 유용성은 요구사항에 따라 다를 수 있습니다. 예를 들어 규모가 크거나 중요한 부분은 이미 사내 시스템과 엔지니어링 팀에서 관리하고 있을 것입니다.
	-	[Machine Learning](https://aws.amazon.com/machine-learning/): 기계학습(Machine learning) 모델 훈련 및 분류
	-	⛓🕍[Data Pipeline](https://aws.amazon.com/datapipeline/): ETL 서비스 관리(용)
	-	⛓🕍[SWF](https://aws.amazon.com/swf/): 관리되는 분산 백그라운드 작업의 구축 및 상태 추적
	-	⛓🕍[Lumberyard](https://aws.amazon.com/lumberyard/): 3D 게임 엔진
-	**모바일/앱 개발:**
	-	[SNS](https://aws.amazon.com/sns/): 앱 푸시 및 기타 사용자 알림 관리
...
| 서비스                         | AWS                                                                          | 구글 클라우드                   | 구글 내부         | 마이크로소프트 애저                     | 기타 제공자                          | 오픈 소스 “스스로 구축”                                        |
|-------------------------------|------------------------------------------------------------------------------|------------------------------|-----------------|------------------------------------|-----------------------------------|------------------------------------------------------------|
| 가상 서버                       | EC2                                                                          | Compute Engine (GCE)         |                 | Virtual Machine                    | DigitalOcean                      | OpenStack                                                  |
| PaaS                          | 엘라스틱 빈스토크                                                                | App Engine                   | App Engine      | Web Apps                           | Heroku                            | Meteor, AppScale                                           |
| 서버리스, 마이크로서비스           | 람다, API 게이트웨이                                                             | Functions                    |                 | Function Apps                      |                                   |                                                            |
| 컨테이너, 클러스터 매니저          | ECS                                                                          | Container Engine, Kubernetes | Borg or Omega   | Container Service                  |                                   | Kubernetes, Mesos, Aurora                                  |
| 파일 스토리지                    | S3                                                                           | Cloud Storage                | GFS             | Storage Account                    |                                   | Swift, HDFS                                                |
| 블록 스토리지                    | EBS                                                                          | Persistent Disk              |                 | Storage Account                    |                                   | NFS                                                        |
| SQL 데이터저장소                 | RDS                                                                          | Cloud SQL                    |                 | SQL Database                       |                                   | MySQL, PostgreSQL                                          |
| 분산된 관계형 디비                |                                                                              |                              | F1, Spanner     |                                    |                                   | Crate.io, CockroachDB                                      |
| 빅테이블                        |                                                                              | Cloud Bigtable               | Bigtable        |                                    |                                   | HBase                                                      |
| 키-밸류 저장소, 컬럼 저장소        | 다이나모디비                                                                     | Cloud Datastore              | Megastore       | Tables, DocumentDB                 |                                   | Cassandra, CouchDB, RethinkDB, Redis                       |
| 메모리 캐시                     | 엘라스틱캐시                                                                    | App Engine Memcache          |                 | Redis Cache                        |                                   | Memcached, Redis                                           |
| 검색                           | 클라우드서치, 엘라스틱서치 (관리되는)                                                |                              |                 | Search                             | Algolia, QBox                     | Elasticsearch, Solr                                        |
| 데이터 웨어하우스                 | 레드쉬프트                                                                     | BigQuery                     | Dremel          | SQL Data Warehouse                 | Oracle, IBM, SAP, HP, many others | Greenplum                                                  |
| 비즈니스 인텔리전스(BI)           | 퀵사이트                                                                       |                              |                 | Power BI                           | Tableau                           |                                                            |
| 락 매니저                       | [다이나모디비 (weak)](https://gist.github.com/ryandotsmith/c95fd21fab91b0823328) |                              | Chubby          | Lease blobs in Storage Account     |                                   | ZooKeeper, Etcd, Consul                                    |
| 메시지 브로커                    | SQS, SNS, IoT                                                                | Pub/Sub                      | PubSub2         | Service Bus                        |                                   | RabbitMQ, Kafka, 0MQ                                       |
| 스크리밍, 분산 로그               | 키네시스                                                                      | Dataflow                     | PubSub2         | Event Hubs                         |                                   | Kafka Streams, Apex, Flink, Spark Streaming, Storm         |
| 맵리듀스                        | EMR                                                                          | Dataproc                     | MapReduce       | HDInsight, DataLake Analytics      | Qubole                            | Hadoop                                                     |
| 모니터링                        | 클라우드와치                                                                   | Monitoring                   | Borgmon         | Monitor                            |                                   | Prometheus(?)                                              |
| 메트릭 관리                     |                                                                              |                              | Borgmon, TSDB   | Application Insights               |                                   | Graphite, InfluxDB, OpenTSDB, Grafana, Riemann, Prometheus |
| CDN                           | 클라우드프론트                                                                   | Cloud CDN                    |                 | CDN                                |                                   | Apache Traffic Server                                      |
| 부하 분산기(Load Balancer)      | CLB/ALB                                                                      | Load Balancing               | GFE             | Load Balancer, Application Gateway |                                   | nginx, HAProxy, Apache Traffic Server                      |
| DNS                           | 라우트53                                                                      | DNS                          |                 | DNS                                |                                   | bind                                                       |
| 이메일                         | SES                                                                          |                              |                 |                                    | Sendgrid, Mandrill, Postmark      |                                                            |
| 깃 호스팅                       | 코드커밋                                                                   | Cloud Source Repositories    |                 | Visual Studio Team Services        | GitHub, BitBucket                 | GitLab                                                     |
| 유저 인증                       | Cognito                                                                      |                              |                 | Azure Active Directory             |                                   | oauth.io                                                   |
| 모바일 앱 분석                   | 모바일 애널리틱스                                                             | Firebase Analytics           |                 | HockeyApp                          | Mixpanel                          |                                                            |
| 모바일 앱 테스팅                 | 디바이스팜                                                                  | Firebase Test Lab            |                 | Xamarin Test Cloud                 | BrowserStack, Sauce Labs, Testdroid                                                            |
